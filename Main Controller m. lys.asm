...
LYSBEHANDLING	BSF		PORTA,6			; Tænd for bit 6, som tænder for den kraftige LED
				CALL	VENT			; Vent en smule tid, så lyset bemærkes
...
LOOP_AD_LYD		BTFSC	ADCON0,2	
				...
				SUBLW 	d'170' 			;Do 122  ADRES, i.e. 122-W
...
CHECKLYS		CLRF	ADRESH
				MOVLW	B'00001000'		;Configurer ADCON og sluk denne
				MOVWF	ADCON0			;
				BSF		ADCON0,0		;Tænd ADCON - ADON = 1
				BSF 	ADCON0,2		;Starter adconverteringen og venter til denne er lav igen
LOOP_AD_LYS		BTFSC	ADCON0,2	
				GOTO	LOOP_AD_LYS	
				MOVFW 	ADRESH 			;Flyt Analog resultat ind i W
				SUBLW 	d'135' 			;Laver 200  ADRES, i.e. 200-W
				BTFSC 	STATUS,Carry 	;Check the carry bit. Clear if ADRES>122
				RETURN			 		;Routine to turn off LED
				CALL 	LYSBEHANDLING	;Routine to turn on LED
				RETURN
